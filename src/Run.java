...
//#####################enter debug mode
		...
		while(cpu.STATE_RUN && debug)
		{
			...
			else if(doption.equals("mem"))
			{
				...
				for(int i=0;i<len;i++)
				{
					System.out.format("address: 0x%x, value: 0x%x char:%c\n",address,cpu.memory.getByte(address),(char)memory.getByte(address));
	...
	//				cpu.memory.printlnDebug();
		...
		if(onlyrun)
		{
	        ...
	        long ms = (between - day * 24 * 60 * 60 * 1000 - hour * 60 * 60 * 1000
	                - min * 60 * 1000 - s * 1000);
	        System.out.println("Total run time : "+day + "Day " + hour + "Hour " + min + "min " + s + "s " + ms
	                + "ms");
		}
